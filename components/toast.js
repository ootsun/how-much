...
export function Toast({message, setMessage}) {

  const [show, setShow] = useState(false);
  const [firstDisplay, setFirstDisplay] = useState(true);

  if(message && firstDisplay) {
    setShow(true);
  ...
  return (
    <div id="toast-success"
         className={`${show ? '' : 'hidden'} z-10 fixed bottom-0 right-0 flex items-center w-full max-w-xs p-4 mb-4 mr-5 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800`}
         role="alert">
      <div
